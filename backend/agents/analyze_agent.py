...
class AnalyzeAgent:
    ...
    def run(
        ...
        # ========== STRATEGY 1.5: Direct import scan (ModuleNotFoundError) ==========
        # Catches broken imports that pass py_compile (valid syntax, missing module)
        import_errors = self._scan_broken_imports(repo_path)
        for err in import_errors:
            key = (err.file, err.line_number)
            if key not in seen:
                seen.add(key)
                errors.append(err)
                logger.info(f"  [IMPORT] {err.bug_type}: {err.file}:{err.line_number} — {err.message[:80]}")
    ...
    def _parse_pytest_output(self, text: str, repo_path: str) -> list[ErrorInfo]:
        ...
        # Pattern 1: pytest FAILURES or ERRORS block
        failure_blocks = re.split(r'_{5,}\s+(.+?)\s+_{5,}', text)
        ...
        for block in failure_blocks:
            ...
            # Walk UP the stack until we find a file that is ACTUALLY in the user's repo!
            # (Ignore deep frames in site-packages or Python's standard library)
            file_path = None
            line_num = 0
            for frame_file, frame_line in reversed(frame_refs):
                fpath = frame_file.replace("\\", "/")
                fpath_lower = fpath.lower()
                if "site-packages" in fpath_lower or "lib/python" in fpath_lower or "python3.14" in fpath_lower or "python312" in fpath_lower or "importlib" in fpath_lower or "pluggy" in fpath_lower:
                    continue
                file_path = fpath
                line_num = int(frame_line)
                break
    ...
    # ==================== STRATEGY 1.5: Broken import scan ====================

    def _scan_broken_imports(self, repo_path: str) -> list[ErrorInfo]:
        """Scan all .py source files for import statements that resolve to non-existent modules.
        
        This catches ModuleNotFoundError at analysis time — completely independent of
        how pytest formats its error output (which varies wildly on Windows/Python 3.12).
        Uses importlib.util.find_spec() which is the same mechanism Python itself uses.
        """
        ...
        # Build set of local module names in this repo so we don't flag them as broken
        local_modules: set[str] = set()
        for py_file in repo.rglob("*.py"):
            parts = py_file.relative_to(repo).parts
            if any(p.startswith(".") or p in ("__pycache__", "venv", ".venv", "node_modules", ".git") for p in parts):
                continue
            # e.g. src/utils.py -> 'src', 'utils', 'src.utils'
            stem = py_file.stem
            local_modules.add(stem)
            module_path = ".".join(parts).removesuffix(".py")
            local_modules.add(module_path)
            # Also add the top-level package name
        ...
        for py_file in repo.rglob("*.py"):
            ...
            if any(p.startswith(".") or p in ("__pycache__", "venv", ".venv", "node_modules", ".git") for p in parts):
                continue
            # Skip test files — they import user code which might validly fail
            if py_file.name.startswith("test_") or py_file.name.endswith("_test.py"):
                continue
            if py_file.name == "__init__.py":
                continue

            # Skip files that don't compile — syntax errors reported separately
            try:
                py_compile.compile(str(py_file), doraise=True)
            ...
            for i, line in enumerate(lines, 1):
                ...
                module_name = None
                if m_import:
                    module_name = m_import.group(1).split(".")[0]  # top-level
                elif m_from:
                    module_name = m_from.group(1).split(".")[0]

                if not module_name:
                    continue

                # Skip if it's a local file in this repo
                if module_name in local_modules:
                    continue

                # Skip known stdlib / built-ins  
                stdlib_top = {
                    "os", "sys", "re", "math", "json", "time", "datetime", "pathlib",
                    "collections", "itertools", "functools", "typing", "abc", "io",
                    "logging", "subprocess", "threading", "multiprocessing", "copy",
                    "random", "string", "struct", "hashlib", "base64", "uuid",
                    "contextlib", "dataclasses", "enum", "warnings", "traceback",
                    "inspect", "ast", "dis", "gc", "weakref", "importlib", "pkgutil",
                    "unittest", "pytest", "socket", "http", "urllib", "email",
                    "html", "xml", "csv", "sqlite3", "pickle", "shelve", "shutil",
                    "tempfile", "glob", "fnmatch", "stat", "platform", "signal",
                    "builtins", "_thread", "queue", "heapq", "bisect", "array",
                    "pprint", "textwrap", "difflib", "argparse", "configparser",
                    "py_compile", "compileall", "zipfile", "tarfile", "gzip",
                    "zlib", "bz2", "lzma", "decimal", "fractions", "statistics",
                    "cmath", "numbers", "operator", "functools", "types",
                }
                if module_name in stdlib_top:
                    continue

                # Use importlib.util.find_spec to check if the module can be found
                try:
                    spec = importlib.util.find_spec(module_name)
                    if spec is not None:
                        continue  # Module exists — fine
                except (ModuleNotFoundError, ValueError):
                    pass  # find_spec itself can raise when module is broken
                except Exception:
