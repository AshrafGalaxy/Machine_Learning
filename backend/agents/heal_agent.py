...
class HealAgent:
    ...
    def _get_strategies(self, bug_type: str, error: ErrorInfo, repo_path: str) -> list:
        ...
        if bug_type == "SYNTAX":
            ...
            return [
                ("remove_bad_import", self._fix_remove_bad_import),
    ...
    def _fix_type_error(self, error: ErrorInfo, lines: list[str], line_num: int) -> str | None:
        ...
        # Case 1: any numeric type (int/float) concatenated with str using + operator
        # Matches: "unsupported operand type(s) for +: 'float' and 'str'"
        #       or "can only concatenate str (not "int") to str"
        numeric_str_concat = (
            ("str" in msg and "int" in msg) or
            ("str" in msg and "float" in msg) or
            ("can only concatenate str" in msg) or
            ("unsupported operand type" in msg and "str" in msg and "+" in line)
        )
        if numeric_str_concat and "+" in line:
            # Strategy A: RHS of + is a numeric string LITERAL like + "1" → convert to numeric
            # e.g. float(val) + "1"  →  float(val) + 1.0
            m_numeric_str = re.search(r'(\+\s*)(["\'])(\d+(?:\.\d+)?)\2', line)
            if m_numeric_str:
                num_str = m_numeric_str.group(3)
                # Determine if result should be float (if lhs has 'float') or int
                is_float_ctx = "float" in line.lower()
                replacement = f"{num_str}.0" if is_float_ctx else num_str
                return line.replace(m_numeric_str.group(0), f" + {replacement}", 1)

            # Strategy B: the RHS of + is a string literal like + "1" and lhs is float() expr
            # wrap the whole lhs in str() so concatenation works
            m_str_rhs = re.search(r'^(\s*return\s+)(.+?)(\s*\+\s*["\'].+)$', line)
            if m_str_rhs:
                indent_ret = m_str_rhs.group(1)
                lhs = m_str_rhs.group(2).strip()
                rhs = m_str_rhs.group(3)
                if not lhs.startswith("str("):
                    return f"{indent_ret}str({lhs}){rhs}\n"

            # Strategy C: LHS is numeric, RHS is string var → wrap RHS in str()
            m_var_rhs = re.search(r'(\+\s*)(\w+)', line)
            if m_var_rhs:
                return re.sub(
    ...
    def _fix_remove_bad_import(self, error: ErrorInfo, lines: list[str], line_num: int) -> str | None:
        """Remove an import line that refers to a genuinely nonexistent module.
        
        Specifically targets ModuleNotFoundError. Removes the import entirely
        by blanking the line (preserving line numbers for subsequent errors).
        Only fires when the error message explicitly says ModuleNotFoundError.
        """
        ...
        # Double-check the module truly doesn't exist before removing it
        try:
            spec = importlib.util.find_spec(module_name)
            if spec is not None:
                return None  # Module exists — don't remove it
        except (ModuleNotFoundError, ValueError):
            pass  # Confirmed missing
        except Exception:
            return None  # Can't determine — be conservative

        logger.info(f"  Removing genuinely missing import: '{module_name}' on line {line_num}")
        return "\n"  # Blank the line
