...
class DiscoverAgent:
    ...
    def _parse_test_counts(
        ...
        if framework == "pytest":
            ...
            # Only apply the catastrophic-crash fallback when NOTHING was parsed at all
            # (e.g. "collected 0 items / 1 error" with no summary line)
            if total == 0 and "error" in combined.lower():
                failed = 1
                total = 1
        ...
        # Fallback: try generic patterns ONLY when framework-specific parsing found nothing
        if total == 0:
            passed_words = len(re.findall(r'\bPASSED\b', combined))
            failed_words = len(re.findall(r'\bFAILED\b', combined))
            if passed_words > 0 or failed_words > 0:
                passed = passed_words
                failed = failed_words
                total = passed + failed
