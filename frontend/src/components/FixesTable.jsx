...
function SortArrow({ active, direction }) {
    if (active) {
        return direction === 'asc'
            ? <FiChevronUp className="inline text-emerald-400" size={14} />
            : <FiChevronDown className="inline text-emerald-400" size={14} />;
    }
            ...
            >
                <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.05] rounded-3xl p-6 sm:p-8 shadow-2xl">
                    <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                        ...
                        Fixes Applied
                        <span className="bg-emerald-500/20 text-emerald-400 text-xs px-2.5 py-0.5 rounded-full font-medium">0</span>
                    </h3>
        ...
        >
            <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.05] rounded-3xl p-6 sm:p-8 shadow-2xl">
                ...
                ${fixFilterType === type
                                    ...
                                    : 'border border-white/[0.05] text-slate-400 hover:border-emerald-500 hover:text-emerald-400'
                                }`}
                        >
                            {type.replace('_', ' ')}
                        </button>
                ...
                {/* Table */}
                <div className="overflow-x-auto rounded-xl border border-white/[0.05] -mx-2 sm:mx-0">
                            ...
                            <tr className="bg-slate-800/30">
                                {COLUMNS.map((col) => (
                                    <th
                                        ...
                                        className={`${col.align} py-3 px-4 text-slate-500 font-medium uppercase text-xs tracking-wider
                                            cursor-pointer select-none hover:text-emerald-400 transition-colors group`}
                                    >
                        ...
                        <tbody>
                            {sorted.map((fix, i) => (
                                <motion.tr
                                    ...
                                    className={`border-t border-white/[0.02] transition-colors duration-200 hover:bg-emerald-500/5
                    ${i % 2 === 0 ? 'bg-transparent' : 'bg-slate-800/10'}`}
                                >
                                    <td className="py-3 px-4 font-mono text-xs text-emerald-400">{fix.file}</td>
                                    <td className="py-3 px-4">
                                        <span className={`inline-block px-2 py-0.5 rounded-md text-xs font-medium border ${BADGE_COLORS[fix.bug_type] || 'bg-slate-500/15 text-slate-400 border-slate-500/30'}`}>
                                            {fix.bug_type}
                                        </span>
                                    </td>
                                    <td className="py-3 px-4 font-mono text-xs text-slate-400">{fix.line_number}</td>
                                    <td className="py-3 px-4 text-xs text-slate-400 max-w-xs truncate">
                                        <CommitMessage message={fix.commit_message} />
                                    </td>
                                    <td className="py-3 px-4 text-center">
                                        {fix.status === 'VERIFIED' ? (
                                            <CheckCircle className="w-4 h-4 text-emerald-400 inline-block" title="Verified" />
                                        ) : fix.status === 'APPLIED' || fix.status === 'PENDING' ? (
                                            <Wrench className="w-4 h-4 text-cyan-400 inline-block" title="Applied" />
                                        ) : (
                                            <XCircle className="w-4 h-4 text-red-500 inline-block" title="Failed" />
                                        )}
                                    </td>
                                </motion.tr>
                            ))}
...
function CommitMessage({ message }) {
    if (!message) return <span className="text-slate-500 italic">No message</span>;
    const prefix = '[AI-AGENT]';
    if (!message.startsWith(prefix)) return <span>{message}</span>;

    return (
        <span>
            <span className="text-emerald-400 font-semibold">{prefix}</span>
